...
k = 2 # increase in temperature (degrees) due to shivering 
l = 4 # decrease in temperature (degrees) due to the cold shower
...
class TemperatureEnvironment:
    ...
    def transition_state(self, current_state):
        ...
        if current_state == 0:
            # stochastic transition, with probability 0.9 the rat remains to the initial state
    ...
    def step(self, action):
        ...
        # state 2 causes temperature drop because of cold shower
        if current_state == 2:
            # during the first time unit after entering state 2:
            # temperature drops by l
...
for run in range(n_runs):
    ...
    for t in range(time_units):
        ...
        # record stats (your event-time indexing)
